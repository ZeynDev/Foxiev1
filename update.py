...
if ospath.exists('Fox_Logs.txt'):
    with open('Fox_Logs.txt', 'r+') as f:
        f.truncate(0)
...
if len(UPSTREAM_REPO) == 0:
    UPSTREAM_REPO = 'https://github.com/ZeynDev/Foxiev1'
...
if UPSTREAM_REPO:
    ...
    update = srun([f"git init -q \
                     && git config --global user.email zeynmoryen@gmail.com \
                     && git config --global user.name ZeynDev \
                     && git add . \
    ...
    if update.returncode == 0:
        ...
        log_info('Thanks For Using @EritsuCH')
    else:
